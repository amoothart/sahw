...
public class App extends Application<AppConfiguration> {
    ...
    public void run(AppConfiguration appConfiguration, Environment environment) throws Exception {
        ...
        for(Collection collection : appConfiguration.getCollections()) {
            registerResource(environment, collection, appConfiguration, httpClient);
            registerHealthCheck(environment, collection, appConfiguration, httpClient);
        }
    }

    private void registerResource(Environment environment, Collection collection, AppConfiguration appConfiguration, HttpClient httpClient) {
        logger.info("Loading resource " + collection.getName());

        Optional<Object> resource = loadResource(collection, appConfiguration, httpClient);
    ...
    private void registerHealthCheck(Environment environment, Collection collection, AppConfiguration appConfiguration, HttpClient httpClient) {
        logger.info("Loading HealthCheck for resource " + collection.getName());

        Optional<HealthCheck> healthCheck = loadHealthCheck(collection, appConfiguration, httpClient);
    ...
    private Optional<Object> loadResource(Collection collection, AppConfiguration appConfiguration, HttpClient httpClient) {
        ...
        try {
            ...
            if(collection.isUsingGlobalHttpClient()) {
                o = loadObject(className, appConfiguration, httpClient);
            } else {
                o = loadObject(className, appConfiguration);
            }
    ...
    private Optional<HealthCheck> loadHealthCheck(Collection collection, AppConfiguration appConfiguration, HttpClient httpClient) {
    ...
    private Object loadObject(String className, AppConfiguration appConfiguration) throws IOException {
        try {
            Constructor<?> constructor = findConstructor(loadClass(className), false);
            return constructor.newInstance(appConfiguration);
        } catch (IOException e) {
            throw e;
        } catch (Exception e) {
    ...
    private Object loadObject(String className, AppConfiguration appConfiguration, HttpClient httpClient) throws IOException {
        try {
            Constructor<?> constructor = findConstructor(loadClass(className), true);
            return constructor.newInstance(appConfiguration, httpClient);
        } catch (IOException e) {
            throw e;
        } catch (Exception e) {
            throw new IOException(e);
    ...
    private Constructor<?> findConstructor(Class clazz, boolean needsHttpClass) throws IOException {
        ...
        try {
            if(needsHttpClass) {
                constructor = clazz.getConstructor(AppConfiguration.class, HttpClient.class);
            } else {
                constructor = clazz.getConstructor(AppConfiguration.class);
            }
